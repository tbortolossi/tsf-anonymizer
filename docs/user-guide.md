...
New to tech support files? [TSF-GUIDE.md](TSF-GUIDE.md) explains what they
...
- **Delete the original after a clean verification** (on by default): once
  ...
  the job says why.
- **Redact binary files that embed identifiers** (off by default): binary
...
| `TSF_COMPARE_WORKERS` | cores left over | processes for one job's verification |
