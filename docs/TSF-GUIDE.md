...
```
./tmp/cli/techsupport_<model>_<date>.txt   ← START HERE: all show/debug output
...
| routing | `routed.log` (legacy) or `frr_export.log` + `etc/frr/` (advanced routing) | `> show routing …` / `> show advanced-routing …` |
...
Binary files (`rule-hit-count.bin`, `*.dat`, sqlite DBs) are copied through
unchanged; the integrity report lists any that still embed identifiers, so
you can drop them from the archive if that matters.
